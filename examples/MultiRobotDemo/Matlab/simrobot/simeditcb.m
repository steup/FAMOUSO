...
%                  matrix = matrprep([pathname filename]);
  % --------------------------------------------
            matrix_bmp=imread([pathname filename],'bmp');
            ...
            %% Is only to produce logical data types !!!
            matrix=~matrix;
            matrix=~matrix;
...
%  % --------------------------------------------% 
      ...
      case 'loadfile'
         ...
         % ******* Enable disabled context menu items ********
         h = findobj('typ','uimenu','Enable','off');
         set(h,'Enable','on')
			% ***************************************************
  	       % ***********************************************************
      ...
      case 'save'
         ...
         if ~isempty(path)
            loadstr = load(path);			% Load (matrix) from original file
 				[pathn,file,ext,ver] = fileparts(path);
 				file = [file ext];
         	% ***************************************************************************
      ...
      case 'saveas'
         ...
         path = get(h,'UserData');
         loadstr = load(path);
         [path,name,ext,ver] = fileparts(path);
