...
namespace famouso{
	...
	void UDPMultiCastNL::init() {
		...
		// socket_.set_option( boost::boost::asio::ip::multicast::hops( 3 ) );
		// socket_.set_option( boost::boost::asio::ip::multicast::enable_loopback(false) );
		socket_.set_option( boost::asio::ip::multicast::enable_loopback( true ) );

		socket_.bind( endpoint_listen );

		socket_.async_receive_from(
			boost::asio::buffer(buffer_, info::payload), endpoint_from,
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	void UDPMultiCastNL::bind( const Subject &s, SNN &snn ){
		...
		socket_.set_option( boost::asio::ip::multicast::join_group( snn ) );
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	void UDPMultiCastNL::deliver(const Packet_t& p) {
		...
		buffers.push_back(boost::asio::buffer( &s, sizeof( Subject )));
